...
class LaporanPenjualanService
{
    ...
    public function produkTerlarisHariIni(int $limit = 5)
    {
        return DB::table('item_penjualan')
            ...
            ->where('penjualan.status', 'COMPLETED') // Diperbaiki: Ditambahkan tanda petik tunggal agar terbaca sebagai string text
            ->groupBy('produk.id', 'produk.nama', 'produk.stok') // Diperbaiki: Menambahkan 'produk.stok' ke dalam groupBy agar lolos aturan SQL ketat
            ->select(
                'produk.nama',
                'produk.stok',
